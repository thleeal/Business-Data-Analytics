...
df = read.csv("facebook.csv",stringsAsFactors = TRUE)
...
where_na = which(is.na(df$Click))
copy = copy(df)

copy <- copy %>%
  group_by(Type) %>%
  mutate(Click = ifelse(is.na(Click), mean(Click, na.rm = TRUE), Click)) %>%
  ungroup()
...
print(copy[where_na,])
print(df[where_na,])
print(copy[where_na,] - df[where_na,])
...
# attract more clicks. For example, the company should post more "Photo" and "Status" posts, decrease the

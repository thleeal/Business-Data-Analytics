...
# Load the data
taxi_data <- read.csv("/Users/tszholee/Downloads/isom 3530/Assignment 2/taxi.csv", stringsAsFactors = TRUE)
...
# Check if the predictors are significant in the model

reduced_model_TimeQ <- lm(PickupCount ~ bs(TimeMin, knots = knots) + isWeekDay, data = taxi_data)
anova(reduced_model_TimeQ, model.5)
# Since the partial F-test p-value is less than 0.05, we can reject the null hypothesis that the TimeQ term does
# not contribute significantly to the model. Therefore, the TimeQ term is significant in the model.

reduced_model_isWeekDay <- lm(PickupCount ~ bs(TimeMin, knots = knots) + TimeQ, data = taxi_data)
anova(reduced_model_isWeekDay, model.5)
# Since the partial F-test p-value is less than 0.05, we can reject the null hypothesis that the isWeekDay term does
# not contribute significantly to the model. Therefore, the isWeekDay term is significant in the model.
...
# pickup count is sensitive to TimeMin and the effect of the time of the day on the  pickup count is not linear.
# Additionally, the pickup count is sensitive to isWeekDay. For example, the pickup count will increase during
# weekends compared to weekdays. Also, the pickup count is sensitive to TimeQ, for instance, the pickup count in
# Q4 is higher than that in Q1. The interaction between the time of the day and the day of the week also has a
# significant impact on the pickup count. For instance, the pickup count in TimeQ Q4 and TimeQ Q2 are higher than
# that inTimeQ Q1 on weekends compared to weekdays. Therefore, the GAM model can be used to make predictions and
# optimize pickup services based on the time of the day and the day of the week, leading to better resource 
# allocation and service. For example, the company can adjust the number of drivers and vehicles available during
# TimeQ04 in weekends to meet the demand and improve customer satisfaction.
```